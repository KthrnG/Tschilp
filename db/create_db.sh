...
function datenbank_anlegen() {
  sqlite3 tschilp.sqlite "CREATE TABLE IF NOT EXISTS voegel(scientific_name TEXT PRIMARY KEY, name_en TEXT, name_de TEXT, avibase_url TEXT, wikipedia_url TEXT,img TEXT);"
}
...
function datenbank_fuellen() {
  ...
  for vogel in $(jq -c .[] ../voegel.json); do
    ...
    sqlite3 tschilp.sqlite "INSERT INTO voegel VALUES($scientific_name,$name_en,$name_de,$avibase_url,$wikipedia_url,$img);"
  done
}
